...
const StorePage = () => {
  ...
  const fetchStoresbyDistrict = async (district) => {
    try {
      // 1. Fetch all stores
      console.log("selected District"+district.toLowerCase())
      ...
      const matchedStores = await Promise.all(
        allStores.map(async (store) => {
          if (!store.district) return null;
  
  
          try {
            
              // if (store.district.toLowerCase() === district.toLowerCase()) {
              //   return store;
              // }
            ...
            console.warn(`Failed to filter district ${store.district}`);
          }
